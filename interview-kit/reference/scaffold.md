...
```
- **Not on Ubuntu (Arch, macOS, …)**: the `apt` installs below fail, so use the dev container. Run `./dev.sh ./preflight.sh` (with `DO_TOKEN=<token>` in front the first time; `dev.sh` forwards only that variable), then `./dev.sh bash -c 'cd backend && make install check up-native'`, or `./dev.sh` for a shell. Everything in this section then runs unchanged inside it. Gotchas:
