...
```
**Update: host isn't the interview's Ubuntu 24 container** (your own Arch/macOS/other laptop, where `apt`/`dpkg` are missing and preflight's installs fail). After `into-project.sh`, run everything in a dev container instead: `cd ~/app && ./dev.sh ./preflight.sh` (then `./dev.sh bash -c 'cd backend && make install check up-native'`, or `./dev.sh` for a shell). It is `ubuntu:24.04` with `doctl`, `terraform`, `gh`, `uv`, Postgres and Redis preinstalled, the host's `gh` login passed in as `GH_TOKEN`, `~/.api_keys.env` shared, doctl auth kept in a volume and the API on `:8000`. Run `make`/`uv`/`python` only through `./dev.sh` (the project's `backend/.venv` is shared with the host, and a host-side `uv run` can rewrite it with the wrong interpreter). Servers started inside it (`make run`, `python -m app.worker`) are reachable at `localhost:8000`; in a one-shot `./dev.sh bash -c` use `setsid` to keep them alive, and don't `pkill -f <name>` from a command that contains that name (it kills its own shell). Inside the real interview container skip this; preflight installs `doctl`/`terraform` itself (arch-aware, no `dpkg`).

**Where the DigitalOcean token goes:** into `DO_TOKEN`, once, on the preflight command: `DO_TOKEN=<token> ./preflight.sh` (in the dev container: `DO_TOKEN=<token> ./dev.sh ./preflight.sh`; `dev.sh` forwards it). preflight runs `doctl auth init` with it (stored in the `app-dev-doctl` volume or `~/.config/doctl`) and reuses that for Terraform, so nothing else needs it. Never write it to a file in the repo or echo it. Either route (the user typing `! DO_TOKEN=<token> ...`, or pasting it to the agent) puts it in the chat transcript, because `!` echoes the command. To keep it out, the user runs the command in a separate terminal (`cd ~/app && DO_TOKEN=<token> ./dev.sh ./preflight.sh`) and tells the agent when done. Revoke the token after the session either way.
...
11. **Defense prep (~15-20 min, can overlap with deploy waits)** — refresh `design-decisions.md` against what actually got built, including the rubric walkthrough (§1). Then read through it: that's the user's prep, since the user answers the walkthrough, not the agent. **Update:** also hand over `API.md` (a curl example for every endpoint, run against the live URL) and `scripts/` (one pytest test per endpoint, runnable from the IDE with `make api-tests`), so the user can exercise everything themselves.
