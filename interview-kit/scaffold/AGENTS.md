...
- DO token: only ever as `DO_TOKEN=<token> ./preflight.sh` (or `./dev.sh ./preflight.sh`), run by the user in a separate terminal (keeps it out of the chat; `!` and pasting both expose it) or pasted once. Never echo it, log it or write it to a file.
