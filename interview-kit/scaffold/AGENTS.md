...
- Host isn't Ubuntu (no apt; practice only)? Run commands through `./dev.sh <cmd>` (`.kit/reference/scaffold.md` §4.17).
- DO token: only as `DO_TOKEN=<token> ./preflight.sh`, ideally typed by the user in a separate terminal (pasting it here
  or `!` puts it in the transcript). Never echo it, log it or write it to a file.
